...
export default function CustomImage({ name, size = 32 }:
  {
    name: string,
    size?: number
  }
  ...
  let imageRow = 0, imageCol = 0, title = "oh no";
  ...
  return (
    <Image
      ...
      style={{
        userSelect: "none",
        background: `url('/spritesheet.png') ${col}px ${row}px ${size ? `/ ${size * 14}px ${size * 14}px` : ""
          }`,
      }}
      draggable={false}
    />
